                        ...
                        <div class="panel mb25">
                            <div class="panel-body">
                                ...
                                <div class="rw-class">
                                    ...
                                    <div class="col-md-8">
                                        @php
                                            $diff = \Carbon\Carbon::parse($case->next_date)->diffForHumans();
                                        ...
                                        @endphp
                                        {{ \Carbon\Carbon::parse($case->next_date)->format('d-m-Y') }} <br><span class='next-date {{$class}} '>({{ $diff }})</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                                            ...
                                            <div class="attach-box">
                                                @if($value->attachment)
                                                    @if (strpos($value->attachment, '.png') !== false || strpos($value->attachment, '.jpg') !== false || strpos($value->attachment, '.jpeg') !== false || strpos($value->attachment, '.bmp') !== false)
                                                        ...
                                                        <img width="100" height="100" src="{{ url('attachment')."/".$value->attachment }}">
                                                    @else
                                                        <h5>{{$value->attachment}}</h5>
                                                        @if(strpos($value->attachment, '.pdf') !== false)
                                                            <img width="100" height="100" src="{{ url('images')}}/pdf.png">
                                                        @elseif(strpos($value->attachment, '.dox') !== false)
                                                            <img width="100" height="100" src="{{ url('images')}}/doc.png">
                                                        @elseif(strpos($value->attachment, '.csv') !== false)
                                                            <img width="100" height="100" src="{{ url('images')}}/csv.png">
                                                        @else
                                                            <img width="100" height="100" src="{{ url('images')}}/file.png">
                                                        @endif
                                                    @endif
                                                    <div class="attach-box-action">
                                                        @if(strpos($value->attachment, '.csv') == false)
                                                            <a title="View attachment" target="_blank" href=" {{url('attachment')."/".$value->attachment}}"><i class="fa fa-eye"></i></a>
                                                            <a title="Download attachment" href="{{url('attachment')."/".$value->attachment}}" download=""><i class="fa fa-download"></i></a>
                                                        @else
                                                            <a title="Download attachment" href="{{url('attachment')."/".$value->attachment}}" download=""><i class="fa fa-download"></i></a>
                                                        @endif
                                                    </div>
                ...
                $(document).ready( function () {
                    ...
                    oTable = $('.datatable').dataTable({
                        ...
                        columns:[
                            ...
                            {
                                title: 'Date', data: 'date', name: 'date', width: "7%", render: function ( data, type, full, meta ) {
                                    if(full['date'] == null){
                                        return "N/A";
                                    }
                                    else{
                            ...
                            {
                                title: 'Attachment', data: 'attachment', name: 'attachment', width: "10%", render: function ( data, type, full, meta ) {
                                    if(full['attachment'] == null){
                                        return "N/A";
                                    }
                                    else{
                                        return '<a title="View attachment" target="_blank" href=" {{url("attachment")}}/'+full['attachment']+'"><button class="btn btn-round btn-success">View</button></a>'+
                                            '<a  download="" title="Download attachment" target="_blank" href=" {{url("attachment")}}/'+full['attachment']+'"><button class="btn btn-round btn-success">Download</button></a>';
                                    }
                                },
                                "searchable": false,
                                "orderable": true
                            ...
                            /*{
                                title: 'Action', data: '', name: '', width: "7%", render: function ( data, type, full, meta ) {
                                    ...
                                    var url = "{{url('/case/entries/search/')}}/"+full['id'];
